...
let messageTableColumns: string[] = [];

async function initMessageColumns() {
  try {
    const { data } = await supabase.from('messages').select('*').limit(1);
    if (data && data.length > 0) {
      messageTableColumns = Object.keys(data[0]);
    } else {
      messageTableColumns = ['id', 'sender_bi', 'recipient_bi', 'org', 'preview', 'created_at', 'unread', 'status', 'subject', 'body'];
    }
  } catch (e) {
    messageTableColumns = ['id', 'sender_bi', 'recipient_bi', 'org', 'preview', 'created_at', 'unread', 'status', 'subject', 'body'];
  }
  printInfo(`Colunas detetadas na tabela 'messages': [${messageTableColumns.join(', ')}]`);
}

let lastGeneratedId = Date.now();
function generateUniqueId(): number {
  const nextId = Date.now();
  if (nextId <= lastGeneratedId) {
...
function buildMessagePayload(params: {
  ...
  if (messageTableColumns && messageTableColumns.length > 0) {
    const filtered: any = {};
    for (const col of messageTableColumns) {
      if (col in fullPayload) {
        filtered[col] = (fullPayload as any)[col];
      }
    }
    // Handle legacy protocol_number if present
    if (messageTableColumns.includes('protocol_number') && !('protocol_number' in filtered)) {
      filtered.protocol_number = null;
    }
    return filtered;
  }
  return fullPayload;
...
async function runTestSuite() {
  ...
  await ensureProfile(ADMIN_CODE, 'Dr.IA', 'admin');
  ...
  // =========================================================================================
  // PASSO 4: Testar correspondência bidirecional com a versão ADMIN (Dr.IA)
  // =========================================================================================
  printStep(4, 'Testando canais de interoperabilidade governamental com o Administrador de Sistemas (DRIA)');

  // 4a. Admin para Institucional (AGT)
  printInfo('Simulando envio de Ofício do Admin (Dr.IA) para a Instituição (AGT)...');
  const adminToInstPayload = buildMessagePayload({
    senderBi: ADMIN_CODE,
    recipientBi: INST_CODE,
    org: ADMIN_CODE,
    subject: 'Auditoria de Sincronização SGE - Dr.IA',
  ...
  // 4b. AGT respondendo de volta para o Admin (Dr.IA)
  printInfo('Simulando resposta da Instituição (AGT) de volta para o Admin (Dr.IA)...');
  const instToAdminPayload = buildMessagePayload({
    senderBi: INST_CODE,
    recipientBi: ADMIN_CODE,
    org: INST_CODE,
    subject: 'RE: Auditoria de Sincronização SGE - Dr.IA',
    body: 'Prezada Administração do DRIA, acusamos a receção do vosso ofício de auditoria. Informamos que os testes de carga correram com sucesso e os dados estão 100% integrados.'
  });
  ...
  printInfo('Simulando envio de Alerta oficial do Admin (Dr.IA) para o Cidadão...');
  ...
  printInfo('Simulando resposta do Cidadão de volta para o Admin (Dr.IA)...');
  ...
  printInfo(`\nTotal de mensagens testadas no Supabase: 14 mensagens reais (5 originais, 5 respostas, 4 de fluxos administrativos do DRIA).`);
  printSuccess('Parabéns! O sistema de Dr.IA opera com 100% de integridade, interoperabilidade e resiliência síncrona com o Supabase.');
}
