...
import {
  ...
  AvaliacaoIaContent,
  HospitaisContent,
  HistoricoConsultasContent,
  PrimeirosSocorrosContent,
  HospitalDashboardContent,
  HospitalPacientesContent,
  HospitalHistoricoContent,
  HospitalPerfilContent,
  MinsaDashboardContent,
  MinsaVigilanciaContent,
  MinsaHospitaisContent,
  MinsaEstatisticasContent,
  MinsaRelatoriosContent,
  MinsaConfiguracaoContent,
  MinsaIaContent,
  CitizenQrCodeContent,
  HospitalQrScannerContent,
  TrabalhadoresHospitalContent,
  TrabalhadoresMinsaContent,
} from './components';

import { DriaEvaluation, DriaHospital } from './types/dria';
import { MOCK_HOSPITALS, INITIAL_EVALUATIONS } from './constants/driaMocks';
...
import { driaSyncService } from './services/driaSyncService';
...
function UnusedModalDummy(props: any) {
  return null;
}
...
export default function App() {
  ...
  const [inbox, setInbox] = useState<Message[]>(() => {
    ...
    const saved = localStorage.getItem('dria_inbox');
    let items: Message[] = [];
    if (!saved) {
      items = baseItems;
    } else {
  ...
  const [instInbox, setInstInbox] = useState<Message[]>(() => {
    ...
    const saved = localStorage.getItem('dria_inst_inbox');
    let items: Message[] = [];
    if (!saved) {
      items = baseItems;
    } else {
  ...
  const [sentMessages, setSentMessages] = useState<Message[]>(() => {
    ...
    const saved = localStorage.getItem('dria_sent');
    let items = baseItems;
    if (saved) {
      try {
        items = JSON.parse(saved);
      } catch (e) {
        console.error('Failed to parse dria_sent:', e);
      }
    }
    return items.map(ensureProtocolOnMessage);
  });
  ...
  const [deletedMessageIds, setDeletedMessageIds] = useState<number[]>(() => {
    const saved = localStorage.getItem('dria_deleted_message_ids');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error('Failed to parse dria_deleted_message_ids:', e);
      }
    }
    return [12];
  });
  ...
  const [hiddenMessageIds, setHiddenMessageIds] = useState<number[]>(() => {
    const saved = localStorage.getItem('dria_hidden_message_ids');
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error('Failed to parse dria_hidden_message_ids:', e);
      }
    }
    return [];
  });
  ...
  useEffect(() => {
    localStorage.setItem('dria_deleted_message_ids', JSON.stringify(deletedMessageIds));
  }, [deletedMessageIds]);

  useEffect(() => {
    localStorage.setItem('dria_hidden_message_ids', JSON.stringify(hiddenMessageIds));
  }, [hiddenMessageIds]);
  ...
  const [contacts, setContacts] = useState<Contact[]>(() => {
    if (shouldUseLocalBootstrap()) {
      const saved = localStorage.getItem('dria_contacts');
      if (saved) {
        try {
          return JSON.parse(saved);
        } catch (e) {
          console.error('Failed to parse dria_contacts:', e);
        }
      }
    }
  ...
  const [documents, setDocuments] = useState<Document[]>(() => {
    ...
    const saved = localStorage.getItem('dria_documents');
    let items = baseItems;
    if (saved) {
      try {
        items = JSON.parse(saved);
      } catch (e) {
        console.error('Failed to parse dria_documents:', e);
      }
    }
  ...
  const [notifications, setNotifications] = useState<AppNotification[]>(() => {
    if (shouldUseLocalBootstrap()) {
      const saved = localStorage.getItem('dria_notifications');
      if (saved) {
        try {
          return JSON.parse(saved);
        } catch (e) {
          console.error('Failed to parse dria_notifications:', e);
        }
      }
    }
  ...
  const [bi, setBiLocal] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_bi') || MOCK_SESSION_USER.bi;
    }
  ...
  const [phone, setPhoneLocal] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_phone') || MOCK_SESSION_USER.phone;
    }
  ...
  const [nif, setNifLocal] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_nif') || MOCK_SESSION_USER.nif;
    }
  ...
  const [passport, setPassportLocal] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_passport') || MOCK_SESSION_USER.passport;
    }
  ...
  const [verificationStatus, setVerificationStatus] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_verification_status') || 'Totalmente verificado';
    }
  ...
  const [hasFacialAuth, setHasFacialAuth] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_has_facial_auth') === 'false' ? false : true;
    }
    return true;
  });
  ...
  const [hasTwoFactor, setHasTwoFactor] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_has_two_factor') === 'true';
    }
    return false;
  });
  ...
  const [govPin, setGovPin] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_gov_pin') || '1234';
    }
    return '1234';
  });

  const [profileName, setProfileNameLocal] = useState('Edlasio Galhardo');

  const [userBirthDate, setUserBirthDateLocal] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_birth_date') || MOCK_SESSION_USER.birthDate;
    }
  ...
  const [userFiliation, setUserFiliationLocal] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_filiation') || MOCK_SESSION_USER.filiation;
    }
  ...
  const [userMaritalStatus, setUserMaritalStatusLocal] = useState(() => {
    if (shouldUseLocalBootstrap()) {
      return localStorage.getItem('dria_marital_status') || MOCK_SESSION_USER.maritalStatus;
    }
  ...
  const setProfileName = (val: string | ((prev: string) => string)) => {
    const resolved = 'Edlasio Galhardo';
  ...
  useEffect(() => {
    localStorage.setItem('dria_bi', bi);
  }, [bi]);

  useEffect(() => {
    localStorage.setItem('dria_phone', phone);
  }, [phone]);

  useEffect(() => {
    localStorage.setItem('dria_nif', nif);
  }, [nif]);

  useEffect(() => {
    localStorage.setItem('dria_passport', passport);
  }, [passport]);

  useEffect(() => {
    localStorage.setItem('dria_verification_status', verificationStatus);
  }, [verificationStatus]);

  useEffect(() => {
    localStorage.setItem('dria_has_facial_auth', String(hasFacialAuth));
  }, [hasFacialAuth]);

  useEffect(() => {
    localStorage.setItem('dria_has_two_factor', String(hasTwoFactor));
  }, [hasTwoFactor]);

  useEffect(() => {
    localStorage.setItem('dria_gov_pin', govPin);
  }, [govPin]);

  useEffect(() => {
    localStorage.setItem('dria_profile_name', profileName);
  }, [profileName]);

  useEffect(() => {
    localStorage.setItem('dria_birth_date', userBirthDate);
  }, [userBirthDate]);

  useEffect(() => {
    localStorage.setItem('dria_filiation', userFiliation);
  }, [userFiliation]);

  useEffect(() => {
    localStorage.setItem('dria_marital_status', userMaritalStatus);
  }, [userMaritalStatus]);
  ...
  // DR.IA State Managers — Supabase-first with localStorage fallback
  const [driaEvaluations, setDriaEvaluations] = useState<DriaEvaluation[]>(() => {
    const saved = localStorage.getItem('dria_evaluations');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { console.error(e); }
    }
    return INITIAL_EVALUATIONS;
  });
  const [driaEvalsLoaded, setDriaEvalsLoaded] = useState(false);

  const [driaHospitals, setDriaHospitals] = useState<DriaHospital[]>(() => {
    const saved = localStorage.getItem('dria_hospitals');
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { console.error(e); }
    }
    return MOCK_HOSPITALS;
  });
  const [driaHospitalsLoaded, setDriaHospitalsLoaded] = useState(false);

  const [selectedPatient, setSelectedPatient] = useState<DriaEvaluation | null>(null);

  // Load evaluations from Supabase (async, graceful fallback)
  useEffect(() => {
    if (stage === 'app' && !driaEvalsLoaded) {
      driaSyncService.loadFromSupabase().then(remote => {
        if (remote.length > 0) {
          setDriaEvaluations(prev => { const ids = new Set(prev.map(e => e.id)); return [...remote.filter(e => !ids.has(e.id)), ...prev]; });
        }
      }).finally(() => setDriaEvalsLoaded(true));
    }
  }, [stage, driaEvalsLoaded]);

  // Load hospitals from Supabase (async, graceful fallback)
  useEffect(() => {
    if (stage === 'app' && !driaHospitalsLoaded) {
      driaSyncService.loadHospitalsFromSupabase().then(remote => {
        if (remote.length > 0) {
          setDriaHospitals(prev => { const ids = new Set(prev.map(h => h.id)); return [...remote.filter(h => !ids.has(h.id)), ...prev]; });
        } else {
          // Se não houver hospitais remotos, envia os mock iniciais para Supabase
          MOCK_HOSPITALS.forEach(h => driaSyncService.saveHospital(h).catch(() => {}));
        }
      }).finally(() => setDriaHospitalsLoaded(true));
    }
  }, [stage, driaHospitalsLoaded]);

  // Sync back to localStorage
  useEffect(() => {
    localStorage.setItem('dria_evaluations', JSON.stringify(driaEvaluations));
  }, [driaEvaluations]);

  useEffect(() => {
    localStorage.setItem('dria_hospitals', JSON.stringify(driaHospitals));
  }, [driaHospitals]);

  // Auto-persist hospitals to Supabase when they change
  useEffect(() => {
    if (driaHospitalsLoaded) {
      driaHospitals.forEach(h => driaSyncService.saveHospital(h).catch(() => {}));
    }
  }, [driaHospitals, driaHospitalsLoaded]);

  // ─── Auto-persist new evaluations to Supabase ───
  const addEvaluation = (newEv: DriaEvaluation) => {
    setDriaEvaluations(prev => [newEv, ...prev]);
    driaSyncService.save(newEv).catch(() => {});
  };

  const updateEvaluation = (updated: DriaEvaluation) => {
    setDriaEvaluations(prev => prev.map(ev => ev.id === updated.id ? updated : ev));
    driaSyncService.save(updated).catch(() => {});
  };

  // Handle active tab change when switching appMode
  useEffect(() => {
    if (appMode === 'user') {
      setTab('home');
    } else if (appMode === 'institution') {
      setTab('hospital-dashboard');
    } else if (appMode === 'admin') {
      setTab('minsa-dashboard');
    }
  }, [appMode]);
  ...
  const [theme, setTheme] = useState<'light' | 'dark'>(() => {
    const saved = localStorage.getItem('dria_theme');
  ...
  const DEMO_CREDENTIALS = {
    ...
    institution: {
      identifier: 'HOSP-DRIA-2026',
      email: 'hospital@dria.ao',
      password: 'Inst@2026',
      profileName: 'Edlasio Galhardo',
      phone: '+244 923 000 111',
      nif: '500323456',
      passport: 'HOSP-CL-01',
      birthDate: '24/09/1982',
      filiation: 'Hospital Central de Luanda — Direção Geral',
      maritalStatus: 'Médico de Serviço',
      verificationStatus: 'Profissional de Saúde Verificado',
      hasTwoFactor: true,
      hasFacialAuth: true,
      govPin: '2211',
    },
    admin: {
      ...
      password: 'Admin@2026',
      profileName: 'Edlasio Galhardo',
      phone: '+244 222 301 200',
      nif: '500010203',
      passport: 'MINSA-GTIC-01',
      birthDate: '15/11/1976',
      filiation: 'Gabinete de TI e Comunicação (GTIC)',
      maritalStatus: 'Funcionário Público',
      verificationStatus: 'Administrador Geral',
      hasTwoFactor: false,
      hasFacialAuth: true,
      govPin: '9900',
    }
  ...
  useEffect(() => {
    localStorage.setItem('dria_inbox', JSON.stringify(inbox));
  }, [inbox]);
  ...
  useEffect(() => {
    localStorage.setItem('dria_inst_inbox', JSON.stringify(instInbox));
  }, [instInbox]);
  ...
  useEffect(() => {
    localStorage.setItem('dria_sent', JSON.stringify(sentMessages));
  }, [sentMessages]);
  ...
  useEffect(() => {
    localStorage.setItem('dria_contacts', JSON.stringify(contacts));
  }, [contacts]);

  useEffect(() => {
    localStorage.setItem('dria_documents', JSON.stringify(documents));
  }, [documents]);

  useEffect(() => {
    localStorage.setItem('dria_notifications', JSON.stringify(notifications));
  }, [notifications]);
    ...
    async function loadSupabaseData() {
      try {
        ...
        if (dbProfile && isSubscribed) {
          const currentPreset = DEMO_CREDENTIALS[appMode];
          const isCanonical = bi === currentPreset.identifier;
          const dbNameMismatch = isCanonical && dbProfile.name && dbProfile.name !== currentPreset.profileName;

          if (dbNameMismatch) {
            console.warn(`CADA: Perfil remoto divergente do utilizador canónico para ${appMode}. A repor identidade 'Edlasio Galhardo'.`);
            applyDemoPresetForMode(appMode, false);
            supabaseService.upsertProfile({
              bi: currentPreset.identifier,
              name: currentPreset.profileName,
              phone: currentPreset.phone,
              nif: currentPreset.nif,
              passport: currentPreset.passport,
              birth_date: currentPreset.birthDate,
              filiation: currentPreset.filiation,
              marital_status: currentPreset.maritalStatus,
              role: appMode === 'user' ? 'user' : appMode === 'institution' ? 'institution' : 'admin'
            }).catch(err => console.error('Erro ao restaurar perfil canónico:', err));
          } else {
            setProfileName('Edlasio Galhardo');
        ...
        const sentSenderKey = isInstMode ? institutionCode : isGovMode ? 'DRIA' : bi;
        ...
        const notificationTarget = isGovMode ? 'DRIA' : isInstMode ? institutionCode : bi;
  ...
  const runAuditAndSincronizacaoCompleta = () => {
    ...
    setInbox(prev => {
      ...
      prev.forEach(m => {
        if (!m.org || m.org.trim() === '') {
          m.org = 'MINSA'; // Corrige: atribui emissor padrão
          fixesCount++;
        }
        if (!ids.has(m.id)) {
          ids.add(m.id);
    ...
    // 3. Audit e De-duplicação de Correspondências de Hospital / MINSA
    setInstInbox(prev => {
      const ids = new Set<number>();
      const uniques: Message[] = [];
      prev.forEach(m => {
    ...
    setDocuments(prev => {
      ...
      prev.forEach(d => {
        if (!d.code || d.code.trim() === '') {
          d.code = `DRIA-REP-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
          fixesCount++;
        }
    ...
    const unsubscribe = subscribeToPreload((stats) => {
      if (stats.progress.isCompleted) {
        ...
        if (failed > 0) {
          ...
          addAuditLog(`[Image Preloader] Todas as ${total} imagens publicitárias pré-carregadas e guardadas em cache com sucesso (Sistemas: Cidadão, Hospital, MINSA)`, 'success');
        }
      }
    });
  ...
  const handleSelectMessage = (message: Message) => {
    ...
    if (message.unread) {
      ...
      addAuditLog(`Sincronização: Correspondência ID ${baseId} marcada como lida em todas as visões (Cidadão, Hospital, MINSA)`, 'success');
    }
  ...
  const handleSendMessage = () => {
    ...
    if (!isOnline) {
      ...
      const sendPromise = isOfficialDispatch
        ? supabaseService.sendOfficialMessage(newMessage, composeData.to, isInstMode ? institutionCode : 'DRIA')
        ...
        .then(async () => {
          ...
          if (isOfficialDispatch) {
            await supabaseService.insertNotification({
              title: 'Nova Notificação Oficial',
              message: `${newMessage.preview} foi disponibilizada no seu Portal de Utente Dr.IA.`,
              type: 'info',
              targetTab: 'home'
            }, composeData.to);
          } else {
  ...
  const handleSendDocMessage = () => {
    ...
    if (!isOnline) {
      ...
      const isOfficialDispatch = isInstMode || isGovMode;
      const sendPromise = isOfficialDispatch
        ? supabaseService.sendOfficialMessage(newMessage, docComposeData.to, isInstMode ? institutionCode : 'DRIA')
  ...
  const handleEmitDocument = (doc: Document, notification: AppNotification) => {
    ...
    const newMessage: Message = {
      id: Number(`${Date.now()}${Math.floor(Math.random() * 1000)}`),
      org: doc.issuer.split(' - ')[0], // Get institution code...
  ...
  const handleCreateRequest = (type: string, priority: 'Alta' | 'Média' | 'Baixa' = 'Média') => {
    ...
    const newNotif: AppNotification = {
      ...
      message: `O seu pedido de ${type} foi registado no sistema Dr.IA.`,
      time: 'Agora',
      type: 'info',
      targetTab: 'home'
    };
    ...
    if (!isOnline) {
      ...
      addAuditLog(`Nova solicitação de ${type} enviada`, 'info');
      OfflineManager.createAutomaticBackup();
    }
  };
  ...
  const getPageContentDescription = (currentTab: string) => {
    switch (currentTab) {
      case 'home':
        return `Você está no Painel Principal do DR.IA APP - Triagem Clínica para Angola.
O utilizador ativo é ${profileName} com Bilhete de Identidade ${bi}.
Neste painel, há alertas de saúde pública e um painel lateral onde se listam os Hospitais & Centros de Saúde integrados como o Hospital Américo Boavida, Hospital Josina Machel e Centros Municipais.
Status de verificação da conta: ${verificationStatus}.
Serviços ativos: Triagem por IA em tempo real e encaminhamento clínico integrado.`;
      
      case 'perfil':
        return `Você está na Ficha Pessoal DR.IA do Cidadão.
Ficha clínica do titular:
...
- Nível de Verificação: ${verificationStatus}`;
      ...
      default:
        return 'Página informativa geral do utilizador na plataforma inteligente DR.IA de Angola.';
    }
  };
  ...
  const handleUpdateDocRequest = async (requestId: number, newStatus: 'Aprovado' | 'Rejeitado') => {
    ...
    if (newStatus === 'Aprovado') {
      const newDoc: Document = {
        ...
        validity: 'VITALÍCIO',
        code: `DRIA-${Math.random().toString(36).substring(2, 10).toUpperCase()}`,
  ...
  // Rendering Helpers
  // Hospital de fallback defensivo (caso driaHospitals fique vazio por eliminação ou bug de persistência)
  const currentHospital: DriaHospital = driaHospitals[0] || MOCK_HOSPITALS[0] || {
    id: 'h-fallback', name: 'Hospital Dr.IA', municipality: 'Luanda', distance: '0 km',
    specialties: ['Clínica Geral'], avgWaitTime: '0 min', doctorsCount: 0, hours: '24h', integrationState: 'Ativo'
  };

  // Derivação única e consistente das avaliações do cidadão autenticado.
  // Usa BI (identidade oficial) como chave principal; cai para nome para compatibilidade com triagens antigas.
  const citizenEvaluations = useMemo<DriaEvaluation[]>(() => {
    const norm = (s: string) => (s || '').toString().toLowerCase().trim();
    return driaEvaluations.filter(ev => {
      const biMatch = bi && norm(ev.patientBI || '') === norm(bi);
      const nameMatch = profileName && norm(ev.patientName) === norm(profileName);
      return biMatch || nameMatch;
    });
  }, [driaEvaluations, bi, profileName]);

  // Dados demográficos derivados da mais recente triagem do cidadão (usados no QR Code e afins)
  const latestCitizenEval = citizenEvaluations[0] || null;
  const citizenAge = latestCitizenEval?.patientAge ?? (() => {
    // Tenta inferir idade a partir da data de nascimento no formato dd/mm/aaaa
    const m = /^(\d{2})\/(\d{2})\/(\d{4})$/.exec(userBirthDate || '');
    if (m) {
  ...
  const citizenGender = latestCitizenEval?.patientGender || 'Masculino';
  const citizenMunicipality = latestCitizenEval?.patientMunicipality || 'Luanda';
  const citizenAllergies = latestCitizenEval?.allergies || 'Nenhuma alergia conhecida';
  const citizenDiseases = latestCitizenEval?.diseases || 'Nenhuma doença registada';
  const citizenMedications = latestCitizenEval?.medications || 'Nenhuma medicação registada';
  const citizenEmergencyContact = latestCitizenEval?.emergencyContact || phone;
  ...
  const renderContent = () => {
    switch (tab) {
      ...
      case 'meu-qrcode':
        return (
          <CitizenQrCodeContent
            ...
            municipality={citizenMunicipality}
            phone={phone}
            allergies={citizenAllergies}
            diseases={citizenDiseases}
            medications={citizenMedications}
            emergencyContact={citizenEmergencyContact}
            evaluations={citizenEvaluations}
            setTab={setTab}
          />
        );
      case 'avaliacao-ia':
        return (
          <AvaliacaoIaContent 
            onAddTriage={addEvaluation} 
            hospitals={driaHospitals}
            bi={bi}
            profileName={profileName}
            initialAge={citizenAge}
            initialGender={citizenGender}
            initialMunicipality={citizenMunicipality}
            initialEmergencyContact={citizenEmergencyContact}
            initialAllergies={latestCitizenEval?.allergies || ''}
            initialDiseases={latestCitizenEval?.diseases || ''}
            initialMedications={latestCitizenEval?.medications || ''}
            setTab={setTab}
          />
        );
      case 'hospitais': {
        // Identifica a avaliação mais recente ainda não encaminhada do cidadão autenticado
        const myEvals = driaEvaluations.filter(
          ev => !ev.submittedHospitalId && (ev.patientName === profileName || ev.patientBI === bi)
        );
        const latestPending = myEvals[0] || driaEvaluations.find(ev => !ev.submittedHospitalId) || null;
        return (
          <HospitaisContent 
            hospitals={driaHospitals} 
            activeEvaluation={latestPending}
            onSendToHospital={(hospitalId, hospitalName) => {
              if (!latestPending) return;
              const submissionTs = new Date().toLocaleDateString('pt-AO') + ' ' + new Date().toLocaleTimeString('pt-AO', { hour: '2-digit', minute: '2-digit' });
              const updated: DriaEvaluation = {
                ...latestPending,
                submittedHospitalId: hospitalId,
                submittedHospitalName: hospitalName,
                submissionTime: submissionTs,
                doctorStatus: 'Aguardando',
                goToHospital: true,
              };
              setDriaEvaluations(prev => prev.map(ev => ev.id === latestPending.id ? updated : ev));
              driaSyncService.save(updated).catch(() => {});
              addAuditLog(`Triagem #${latestPending.id} encaminhada para ${hospitalName}`, 'success');
            }}
          />
        );
      }
      ...
      // ================= DR.IA HOSPITAL PLATFORM =================
      case 'hospital-qr-scanner':
        return (
          <HospitalQrScannerContent
            evaluations={driaEvaluations}
            setTab={setTab}
          />
        );
      ...
      case 'hospital-perfil':
        return (
          <HospitalPerfilContent 
            hospital={currentHospital} 
            onUpdateHospital={(updated) => setDriaHospitals(prev => prev.map(h => h.id === updated.id ? updated : h))} 
            setTab={setTab}
          />
        );
      case 'hospital-conta':
        return (
          <ProfileContent
            ...
            nif="500323456"
            passport="HOSP-CL-01"
            verificationStatus="Credenciado"
            hasFacialAuth={true}
            hasTwoFactor={true}
            govPin="2211"
            profileName={profileName}
            userEmail="hospital@dria.ao"
            userBirthDate="24/09/1982"
            userFiliation="Hospital Central de Luanda - Direção Geral"
            userMaritalStatus="Médico de Serviço"
            setBi={() => {}}
            setPhone={() => {}}
            setNif={() => {}}
            setPassport={() => {}}
      ...
      case 'hospital-trabalhadores':
        return (
          <TrabalhadoresHospitalContent />
        );

      // ================= DR.IA MINISTÉRIO DA SAÚDE (MINSA) =================
      case 'minsa-dashboard':
        return (
          <MinsaDashboardContent 
            evaluations={driaEvaluations} 
            hospitals={driaHospitals}
      ...
      case 'minsa-vigilancia':
        return (
          <MinsaVigilanciaContent evaluations={driaEvaluations} />
        );
      case 'minsa-hospitals':
        return (
          <MinsaHospitaisContent 
            hospitals={driaHospitals}
            evaluations={driaEvaluations}
            onToggleIntegration={(hId) => setDriaHospitals(prev => prev.map(h => h.id === hId ? { ...h, integrationState: h.integrationState === 'Ativo' ? 'Inativo' : 'Ativo' } : h))} 
          />
        );
      case 'minsa-estatisticas':
        return (
          <MinsaEstatisticasContent
            evaluations={driaEvaluations}
            hospitals={driaHospitals}
      ...
      case 'minsa-configuracao':
        return (
          <MinsaConfiguracaoContent />
        );
      case 'minsa-trabalhadores':
        return (
          <TrabalhadoresMinsaContent />
        );
      case 'minsa-conta':
        return (
          <ProfileContent
            ...
            govPin="8812"
            profileName={profileName}
            userEmail="minsa@dria.ao"
            userBirthDate="15/11/1976"
            userFiliation="Gabinete de TI e Comunicação (GTIC)"
            userMaritalStatus="Funcionário Público"
            setBi={() => {}}
            setPhone={() => {}}
            setNif={() => {}}
            setPassport={() => {}}
        ...
        >
          <div className="flex flex-col items-center justify-center mb-8">
            <LazyImage
              ...
              className="mb-6 shrink-0 transition-all duration-350 hover:scale-[1.02]"
            />
            <h2 className="text-2xl font-black text-slate-900 tracking-tight leading-none mb-2 uppercase italic">DR.IA APP</h2>
            <p className="text-[10px] font-bold text-slate-400 tracking-widest uppercase">{t("Triagem Clínica Inteligente • Angola")}</p>
          </div>
          ...
          <motion.p className="text-slate-500 text-[10px] md:text-xs font-black uppercase tracking-[0.2em] mt-4">
            {t("A carregar Dr.IA — Ministério da Saúde...")}
          </motion.p>
        </motion.div>
      </section>
    );
  ...
  if (stage === 'login') {
    const handleDemoAutofill = () => {
      ...
      setLoginEmail(preset.email || 'demo@dria.ao');
    ...
    const handleLoginSubmit = async () => {
      ...
      try {
        ...
        if (authService.isConfigured() && loginEmail && loginPasswordInput) {
          const { data, error } = await authService.login({ email: loginEmail, password: loginPasswordInput });
          if (error) {
            setLoginError(error);
            addAuditLog(`Falha de autenticação: ${error}`, 'critical');
            setIsLoggingIn(false);
            return;
          }
          if (data?.user) {
            // Load profile from Supabase (highly optimized using direct ID query)
            const profile = await authService.getProfile(data.user.id);
            if (profile) {
              const sessionUser = authService.buildSessionUser(profile);
              updateUserFields(sessionUser);
              setBi(profile.bi || bi);
              setProfileName('Edlasio Galhardo');
              setPhone(profile.phone || phone);
            }
            setStage('app');
            addAuditLog('Login via Supabase Auth — Sucesso', 'success');
            // Seed demo data on first login
            driaSyncService.seedDemoData().catch(() => {});
            setIsLoggingIn(false);
            return;
          }
        }

        // Fallback: legacy demo auth
        if (emergencyMode && !isInstMode && !isGovMode && (bi.toLowerCase().includes('002931298') || bi.toLowerCase().includes('edlasio') || profileName.toLowerCase().includes('edlasio'))) {
          setLoginError("Credenciais suspensas ao abrigo do protocolo SOC-AN-2026.");
          setIsLoggingIn(false);
          return;
        }
        
        // Execute instant transition for highly-responsive local demo auth
        setStage('app');
        addAuditLog('Login via Demo (Supabase não configurado)', 'info');
        setIsLoggingIn(false);
      } catch (err: any) {
        ...
        <div className="max-w-[940px] w-full mx-auto grid grid-cols-1 md:grid-cols-[1.2fr_1fr] gap-4.5 items-stretch">
          ...
          >
            ...
            {showVoiceGuide ? (
              ...
              >
                <UnusedModalDummy /* VoiceGuide removed */
                  onRemoved={() => {
          ...
          <motion.div 
            ...
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: 1, x: 0 }}
            className={`bg-white rounded-3xl ${loginSubMode === 'face-capture' ? 'p-3.5 sm:p-4.5 md:p-5.5 min-h-[410px]' : 'p-4 sm:p-6 md:p-8 min-h-[510px]'} shadow-xl border border-[#E2E8F0] flex flex-col justify-between h-full transition-all duration-300 relative ${
              highlightSteps 
                ...
                >
                  ...
                  <div className="text-center space-y-1.5">
                    ...
                    <p className="text-[10.5px] text-slate-400 font-extrabold uppercase tracking-wider leading-none mt-1">
                      {isInstMode ? t('Canal Oficial do Hospital Dr.IA') : isGovMode ? t('Acesso Reservado — Ministério da Saúde') : t('Acesso Oficial do Cidadão Angolano')}
                    </p>
                  </div>
                  ...
                  <div className="space-y-3.5 pt-1">
                    ...
                    <div className="grid gap-1.5 text-left">
                      <span className="text-[10.5px] text-slate-500 font-extrabold tracking-wider uppercase">
                        {(isInstMode || isGovMode) ? t("Email Institucional") : t("Email de Acesso")}
                      </span>
                      <div className="flex items-center gap-3 bg-white border border-slate-200 focus-within:border-[#0c2340] focus-within:ring-1 focus-within:ring-[#0c2340] rounded-xl px-3 py-1.5 transition-all">
                        ...
                        <input 
                          ...
                          value={loginEmail || bi}
                          onChange={(e) => { setLoginEmail(e.target.value); setBi(e.target.value); }}
                          placeholder={isInstMode ? "hospital@dria.ao" : isGovMode ? "minsa@dria.ao" : "utente@dria.ao"}
                          maxLength={60}
                        />
                      </div>
                    </div>
                    ...
                    <div className="pt-2 flex flex-col gap-2.5">
                      ...
                      <button 
                        ...
                        className="w-full bg-[#0E2B64] hover:bg-[#081a3d] disabled:opacity-80 disabled:cursor-not-allowed text-white rounded-xl py-3 font-black text-[11px] uppercase tracking-wider shadow-[#0E2B64]/15 hover:opacity-95 transition-all cursor-pointer border-none flex items-center justify-center gap-2"
                      >
                        {isLoggingIn ? (
                          <>
                            <Loader2 size={14} className="animate-spin text-white" />
                            <span>{t("A Entrar...")}</span>
                          </>
                        ) : (
                          t("Entrar no Dr.IA")
                        )}
                      ...
                      <div className="space-y-1.5">
                        <p className="text-[10px] text-slate-400 font-extrabold text-center uppercase tracking-wider">
                          {isInstMode ? t('Credenciais de Demonstração — Hospital') : isGovMode ? t('Credenciais de Demonstração — MINSA') : t('CREDENCIAIS DE DEMONSTRAÇÃO — CIDADÃO')}
                        </p>
                      ...
                      <div className="pt-3 mt-1.5 border-t border-slate-100 flex items-center justify-between">
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setAccessModalMessage('Para recuperar a sua senha ou o seu código de segurança de saúde (PIN), por favor dirija-se a um posto de atendimento do Hospital Geral, MINSA ou contacte a linha oficial de suporte do DR.IA de Angola.');
              ...
              >
                ...
                <div className="bg-gradient-to-r from-slate-900 to-indigo-950 p-6 text-white relative">
                  ...
                  <div className="flex items-center gap-3.5">
                    ...
                    <div>
                      <div className="text-[9px] font-black uppercase tracking-[0.2em] text-indigo-300 font-bold">Plataforma Médica DR.IA</div>
  ...
  return (
    <>
      {/* Mobile sidebar overlay (placeholder slot for mobile drawer) */}
      <AnimatePresence>
        {mobileSidebarOpen && (
          <>
            {/* Backdrop */}
            <motion.div
              ...
              className="lg:hidden fixed inset-0 bg-black/60 z-[1000] cursor-pointer"
            />
            {/* Drawer Content */}
            <motion.div
              initial={{ x: '-100%' }}
              ...
              className="lg:hidden fixed inset-y-0 left-0 w-[270px] bg-white z-[1001] shadow-2xl flex flex-col"
            >
              <Sidebar
                tab={tab}
                setTab={setTab}
                ...
                setStage={(s) => {
                  setStage(s);
                  if (s === 'splash') setLoginSubMode('normal');
                }}
                currentLanguage={currentLanguage}
                theme={theme}
      ...
      <AnimatePresence>
        {emergencyMode && isGovMode && (
          <motion.div
            ...
            exit={{ y: -20, opacity: 0 }}
            className="fixed top-0 left-0 right-0 z-[2000] bg-gradient-to-r from-danger-600 via-red-500 to-danger-600 text-white h-8 flex items-center justify-center gap-2 shadow-lg"
          >
            <ShieldAlert size={14} className="animate-pulse" />
            <span className="text-[10px] font-black uppercase tracking-[0.3em]">MODO DE EMERGÊNCIA ACTIVO — OPERAÇÕES RESTRITAS</span>
            <ShieldAlert size={14} className="animate-pulse" />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="app-main min-h-screen">
        <div className={emergencyMode && isGovMode ? 'lg:mt-0' : ''}>
          <Header
            setTab={setTab}
            tab={tab}
            currentLanguage={currentLanguage}
        ...
        <div
          ref={contentRef}
          className={`app-content overflow-y-auto ${emergencyMode && isGovMode ? 'pt-28 lg:pt-2' : (isGovMode ? 'pt-20 lg:pt-2' : 'pt-20 lg:pt-4')}`}
        >
          <div className="max-w-[1440px] mx-auto stagger">
            {renderContent()}
          </div>
        </div>
      </div>
      ...
      {/* AddContactModal removed */} <UnusedModalDummy 
      ...
      {/* InviteConfirmModal removed */} <UnusedModalDummy 
      ...
      {/* DeleteContactModal removed */} <UnusedModalDummy 
                  ...
                  {showSuccessDetails && (
                    ...
                    >
                      ...
                      <div className="flex justify-between items-center">
                        <span className="text-slate-400 font-black tracking-wider uppercase text-[7px]">Nº DRIA/BI:</span>
      ...
      {/* NotificationDetailModal removed */} <UnusedModalDummy
