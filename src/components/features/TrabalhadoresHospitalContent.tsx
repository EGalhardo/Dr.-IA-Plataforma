...
const INITIAL_WORKERS: Worker[] = [
  {
    id: 'HOSP-0024',
    name: 'DR. MANUEL DA SILVA',
    email: 'm.silva@hospital.dria.ao',
    phone: '+244 923 111 024',
    role: 'Médico Pediatra / Chefe de Serviço',
    department: 'BANCO DE URGÊNCIA INFANTIL',
    active: true,
    lastAccess: 'Hoje, 08:30',
  },
  {
    id: 'HOSP-0081',
    name: 'DRA. AMÉLIA ANTUNES',
    email: 'a.antunes@hospital.dria.ao',
    phone: '+244 912 804 081',
    role: 'Médica de Clínica Geral',
    department: 'TRIAGEM E CONSULTAS',
    active: true,
    lastAccess: 'Ontem, 16:40',
  },
  {
    id: 'HOSP-0152',
    name: 'ENF. HAMILTON SANTANA',
    email: 'h.santana@hospital.dria.ao',
    phone: '+244 931 773 152',
    role: 'Enfermeiro Supervisor Geral',
  ...
  },
  {
    ...
    email: 's.sousa@hospital.dria.ao',
    phone: '+244 922 400 431',
    role: 'Técnica Superior de Laboratório',
    department: 'ANÁLISES CLÍNICAS',
    active: false,
    lastAccess: '18/05/2026',
  },
];

export function TrabalhadoresHospitalContent() {
  const { t } = useLanguage();
  const [workers, setWorkers] = useState<Worker[]>(() => {
    const saved = localStorage.getItem('dria_hospital_workers');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('dria_hospital_workers', JSON.stringify(workers));
  }, [workers]);
  ...
  return (
    <div className="space-y-6 text-left font-sans">
      ...
      {/* Top Breadcrumb exactly like image 1 */}
      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-[#0E2B64]">
        <span className="opacity-60">Portal Institucional</span>
        <span>•</span>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        {/* Adicionar Trabalhador Button exactly like image 1 */}
        <button
          onClick={handleOpenAddModal}
          className="bg-[#0E2B64] hover:bg-[#081a3d] text-white rounded-2xl px-5 py-3 font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2.5 cursor-pointer shadow-md shadow-indigo-900/10 active:scale-[0.98] transition-all shrink-0 self-start md:self-center border-none"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-[28px] p-6 shadow-3xs flex flex-col justify-center">
          <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1">Canal Regulamentado</span>
          <div className="flex items-baseline gap-1.5">
            <span className="text-xl font-black text-indigo-600 italic uppercase">AGT</span>
            <span className="text-xs text-indigo-500 font-bold uppercase tracking-wider">Acesso Governamental</span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-[32px] overflow-hidden shadow-xs">
        ...
        <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 border border-slate-200 focus-within:border-[#0E2B64] rounded-xl px-3 py-2 bg-slate-50/50 w-full md:max-w-md">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-xl font-black tracking-tight uppercase text-[#0c2340] italic leading-tight">
                      {editingWorker ? 'EDITAR COLABORADOR HOSPITALAR' : 'REGISTAR NOVO TRABALHADOR DO HOSPITAL'}
                    </h3>
                    <p className="text-[10px] text-blue-600 font-extrabold uppercase tracking-widest mt-1 leading-none">
                      CREDENCIAL OPERACIONAL DO HOSPITAL
                    </p>
                  </div>
                </div>
                <button
              ...
              <form onSubmit={handleSave} className="p-8 space-y-6 overflow-y-auto max-h-[70vh] custom-scrollbar">
                ...
                <div>
                  <div className="flex items-center gap-2 text-indigo-600 font-black text-xs uppercase tracking-wider mb-4">
                    ...
                    <span>DADOS PESSOAIS DO TRABALHADOR</span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    {/* Name */}
                    <div className="grid gap-2">
                      <span className="text-[10px] text-slate-400 font-black tracking-wider uppercase">NOME DO TRABALHADOR *</span>
                    ...
                    {/* Email */}
                    <div className="grid gap-2">
                      <span className="text-[10px] text-slate-400 font-black tracking-wider uppercase">EMAIL HOSPITALAR *</span>
                      <div className="flex items-center gap-2.5 border border-slate-200 focus-within:border-blue-500 rounded-full px-4 py-3 bg-white h-12 shadow-2xs transition-all">
                        <Mail size={16} className="text-slate-400 shrink-0" />
                        <input
                          type="email"
                          required
                          placeholder="Ex: f.manuel@hospital.dria.ao"
                    ...
                    {/* Phone */}
                    <div className="grid gap-2">
                      <span className="text-[10px] text-slate-400 font-black tracking-wider uppercase">TELEFONE DE SERVIÇO *</span>
                      <div className="flex items-center gap-2.5 border border-slate-200 focus-within:border-blue-500 rounded-full px-4 py-3 bg-white h-12 shadow-2xs transition-all">
                ...
                <div>
                  <div className="flex items-center gap-2 text-indigo-600 font-black text-xs uppercase tracking-wider mb-4">
                    ...
                    <span>AFILIAÇÃO & FUNÇÕES NO HOSPITAL</span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                    {/* Role */}
                    <div className="grid gap-2">
                      <span className="text-[10px] text-slate-400 font-black tracking-wider uppercase">ESPECIALIDADE / CARGO *</span>
                      <div className="flex items-center gap-2.5 border border-slate-200 focus-within:border-blue-500 rounded-full px-4 py-3 bg-white h-12 shadow-2xs transition-all">
                        <IdCard size={16} className="text-slate-400 shrink-0" />
                        <input
                          type="text"
                          required
                          placeholder="Ex: Médico de Clínica Geral"
                          value={formData.role}
                    ...
                    {/* Department */}
                    <div className="grid gap-2">
                      <span className="text-[10px] text-slate-400 font-black tracking-wider uppercase">DEPARTAMENTO / SETOR HOSPITALAR *</span>
                      <div className="flex items-center gap-2.5 border border-slate-200 focus-within:border-blue-500 rounded-full px-4 py-3 bg-white h-12 shadow-2xs transition-all">
                        <Building size={16} className="text-slate-400 shrink-0" />
                        <input
                          type="text"
                          required
                          placeholder="Ex: Banco de Urgência, Triagem"
                    ...
                    <div className="grid gap-2">
                      <span className="text-[10px] text-slate-400 font-black tracking-wider uppercase">ID ÚNICO DO TRABALHADOR (GERADO)</span>
