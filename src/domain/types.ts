...
export enum PaymentMethod {
  ...
  CARTEIRA_DIGITAL = "Carteira de Saúde"
}
...
export interface DbInstitution {
  ...
  name: string;             // Short name, e.g. "MINSA"
  fullName: string;         // Full name, e.g. "Administração Geral Tributária"
  category: InstitutionCategory;
  province: string;
  municipio: string;
  status: InstitutionStatus;
  instCode: string;         // Unique registration code, e.g. "MINSA-001"
...
export interface DbDigitalProtocol {
  id: string;               // PK, UUID
  protocolNumber: string;   // Unique format: "DRIA-2026-PT-XXXXXX"
...
export interface DbCorrespondence {
  ...
  org: string;              // Sender visual label, e.g. "MINSA"
  preview: string;
...
export interface DbDocument {
  ...
  issuer: string;           // Issuer institution or department, e.g. "MINSA"
...
export interface DbDocRequest {
  ...
  institution: string;      // Target Institution, e.g. "MINSA"
  requestDate: string;
