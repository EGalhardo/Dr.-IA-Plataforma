...
// Helper to safely handle and log Supabase errors without causing unhandled exceptions or noisy console.errors on network/schema issues
const handleSupabaseError = (error: any, contextName: string) => {
  const msg = error?.message || String(error);
  const isNetworkOrSchema = 
  ...
  if (isNetworkOrSchema) {
    console.warn(`Supabase ${contextName} warning (handled):`, msg);
  } else {
    console.error(`Supabase ${contextName} error:`, error);
  }
  return null;
};
...
export const resolveInstitutionCode = (label?: string): string => {
  ...
  if (normalized.includes('DRIA') || normalized.includes('ADMINISTRAÇÃO CENTRAL')) return 'DRIA';
...
const inferProfileRole = (identifier: string): 'user' | 'institution' | 'admin' => {
  ...
  if (upper === 'DRIA' || upper.startsWith('ADM-')) return 'admin';
...
const deriveProfileName = (identifier: string, fallbackName?: string) => {
  ...
  if (role === 'admin') return 'Administração Central da Plataforma DR.IA Angola';
...
let cachedMessageColumns: string[] | null = null;

async function getMessageColumns(): Promise<string[]> {
  if (cachedMessageColumns) return cachedMessageColumns;
  try {
    const { data } = await supabase.from('messages').select('*').limit(1);
    if (data && data.length > 0) {
      cachedMessageColumns = Object.keys(data[0]);
    } else {
      cachedMessageColumns = ['id', 'sender_bi', 'recipient_bi', 'org', 'preview', 'created_at', 'unread', 'status', 'subject', 'body'];
    }
  } catch (e) {
    cachedMessageColumns = ['id', 'sender_bi', 'recipient_bi', 'org', 'preview', 'created_at', 'unread', 'status', 'subject', 'body'];
  }
...
const filterMessagePayload = async (fullPayload: any) => {
  const cols = await getMessageColumns();
  const filtered: any = {};
  for (const col of cols) {
    if (col in fullPayload) {
      filtered[col] = fullPayload[col];
    }
  }
  if (cols.includes('protocol_number') && !('protocol_number' in filtered)) {
    filtered.protocol_number = fullPayload.protocol_number || null;
  }
  return filtered;
};
...
const createMessagePayload = ({
  ...
  // Só incluir 'id' quando o chamador pedir explicitamente (ex: updates por ID)
  // porque a coluna é BIGSERIAL e deixamos o Postgres gerar para novas mensagens.
  if (includeId && msg.id !== undefined && msg.id !== null) {
    payload.id = Number(msg.id);
  }
  return payload;
};
...
export const supabaseService = {
  ...
  async upsertProfile(profile: {
    ...
    try {
      ...
      handleSupabaseError(e, 'update profile');
      return null;
    }
  },
  ...
  async insertMessage(msg: Message, userBi: string) {
    if (!hasValidSupabaseKeys()) return null;
    try {
      const rawPayload = createMessagePayload({
        msg,
      ...
      const payload = await filterMessagePayload(rawPayload);
      ...
      handleSupabaseError(e, 'insertMessage');
      return null;
    }
  },
  ...
  async sendCitizenMessage(msg: Message, citizenBi: string, institutionLabel: string, citizenName?: string) {
    ...
    try {
      ...
      handleSupabaseError(e, 'sendCitizenMessage');
      return null;
    }
  },
  ...
  async sendOfficialMessage(msg: Message, citizenBi: string, institutionLabel: string) {
    ...
    try {
      ...
      await ensureProfileExists(institutionCode, institutionLabel, institutionCode === 'DRIA' ? 'admin' : 'institution');
      const rawPayload = createMessagePayload({
        msg,
      ...
      handleSupabaseError(e, 'sendOfficialMessage');
      return null;
    }
  },
  ...
  async updateMessageState(messageId: number, changes: Partial<{ unread: boolean; status: string; preview: string; subject: string; body: string; deadline_text: string; state_indicator: string; actions: string[] }>) {
    ...
    try {
      ...
      for (const k of Object.keys(changes)) {
        if (cols.includes(k)) {
          filteredChanges[k] = (changes as any)[k];
        }
      }
      const { data, error } = await supabase.from('messages').update(filteredChanges).eq('id', messageId).select();
      if (error) throw error;
      return data;
    } catch (e: any) {
      handleSupabaseError(e, 'updateMessageState');
      return null;
    }
  },
  ...
  async insertMessageStateEvent({
    ...
    try {
      ...
      handleSupabaseError(e, 'insertMessageStateEvent');
      return null;
    }
  },
  ...
  async getMessageStateHistory(messageId: number): Promise<any[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getMessageStateHistory');
      return null;
    }
  },
  ...
  async insertDocument(doc: Document, userBi: string) {
    ...
    try {
      ...
      handleSupabaseError(e, 'insertDocument');
      return null;
    }
  },
  ...
  async insertContact(contact: Contact, ownerBi: string) {
    ...
    try {
      const payload: any = {
        // id é BIGSERIAL — não enviar para evitar conflitos em upsert
        owner_bi: ownerBi,
        name: contact.name,
        bi: contact.bi,
      ...
      handleSupabaseError(e, 'insertContact');
      return null;
    }
  },
  ...
  async deleteContact(contactId: number) {
    ...
    try {
      ...
      handleSupabaseError(e, 'deleteContact');
      return null;
    }
  },
  ...
  async insertAuditLog(log: { action: string; user: string; timestamp?: string; type?: string }) {
    ...
    try {
      ...
      handleSupabaseError(e, 'auditLog');
      return null;
    }
  },
  ...
  async insertUserRequest(req: UserRequest) {
    ...
    try {
      const payload = {
        // id é BIGSERIAL — não enviar, deixar Postgres gerar
        user_bi: req.bi,
        user_name: req.user,
        ...
        time_text: req.time,
        status: req.status,
        institution: req.institution || 'MINSA'
      };
      const { data, error } = await supabase
        .from('user_requests')
        .upsert([payload])
      ...
      handleSupabaseError(e, 'userRequest');
      return null;
    }
  },
  ...
  async insertDocRequest(req: DocRequest) {
    ...
    try {
      ...
      handleSupabaseError(e, 'docRequest');
      return null;
    }
  },
  ...
  async getProfile(bi: string) {
    ...
    try {
      ...
      handleSupabaseError(e, 'getProfile');
      return null;
    }
  },
  ...
  async getMessages(bi: string): Promise<Message[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getMessages');
      return null;
    }
  },
  ...
  async getInstitutionMessages(institutionLabel: string): Promise<Message[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getInstitutionMessages');
      return null;
    }
  },
  ...
  async getSentMessagesBySender(senderBi: string): Promise<Message[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getSentMessagesBySender');
      return null;
    }
  },
  ...
  async getDocuments(bi: string): Promise<Document[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getDocuments');
      return [];
    }
  },
  ...
  async getContacts(bi: string): Promise<Contact[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getContacts');
      return null;
    }
  },
  ...
  async getNotifications(bi: string): Promise<any[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getNotifications');
      return null;
    }
  },
  ...
  async insertNotification(notif: any, targetBi: string) {
    ...
    try {
      ...
      handleSupabaseError(e, 'insertNotification');
      return null;
    }
  },
  ...
  async getDocRequests(bi?: string): Promise<DocRequest[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getDocRequests');
      return [];
    }
  },
  ...
  async getAuditLogs(): Promise<any[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getAuditLogs');
      return null;
    }
  },
  ...
  async insertCorrespondence(cor: Correspondence) {
    ...
    try {
      ...
      const payload = {
        ...
        org: cor.institution || cor.sender || 'DRIA',
        preview: cor.subject,
        unread: false,
      ...
      handleSupabaseError(e, 'insertCorrespondence');
      return null;
    }
  },
  ...
  async getCorrespondences(): Promise<Correspondence[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getCorrespondences');
      return null;
    }
  },
  ...
  async getDigitalProtocols(): Promise<any[] | null> {
    ...
    try {
      ...
      handleSupabaseError(e, 'getDigitalProtocols');
      return null;
    }
  },
  ...
  async insertDigitalProtocol(p: any) {
    ...
    try {
      ...
      handleSupabaseError(e, 'insertDigitalProtocol');
      return null;
    }
  },
  ...
  async seedAll(params: {
    ...
    try {
      ...
      for (const msg of params.institutionInbox || []) {
        try {
          const targetInstitution = msg.institution || params.institutionCode || 'MINSA';
